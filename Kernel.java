...
public class Kernel
{
    ...
    private static FileSystem fileSystem; //Declare a FileSystem object
    ...
    public static int interrupt( int irq, int cmd, int param, Object args ) {
        ...
        switch( irq ) {
            case INTERRUPT_SOFTWARE: // System calls
                TCB myTcb; //Declare a TCB object
                FileTableEntry ftEnt; //declare a file table entry object
                switch( cmd ) {
                    case BOOT:
                        ...
                        //Instantiate the FileSystem object with 1000 disk blocks
                    ...
                    case READ:
                        switch ( param ) {
                            ...
                            case STDERR:
                                ...
                                return ERROR;
                            default:
                                //Default case is to read from a file
                                if ((myTcb = scheduler.getMyTcb()) != null) { //If the tcb actually exists
                                    ftEnt = myTcb.getFtEnt(param); //Get the file table entry
                                    if (ftEnt != null) { //If the file table entry exists
                                        return fileSystem.read(ftEnt, ((byte[])args)); //Read from the file table entry
                                    }
                                }
                        ...
                        switch ( param ) {
                            ...
                            default:
                                ...
                                if ((myTcb = scheduler.getMyTcb()) != null) { //If the tcb actually exists
                                    ftEnt = myTcb.getFtEnt(param); //Get the file table entry
                                    if (ftEnt != null) { //If the file table entry exists
                                        return fileSystem.write(ftEnt, ((byte[])args)); //Write to the file
                                    }
                                }
                                return ERROR; //Return ERROR on failure
                        }
                    ...
                    case OPEN:    // to be implemented in project
                        if ((myTcb = scheduler.getMyTcb()) != null) { //if the TCB actually exists
                            String[] s = (String[])args;    //Get an array of string arguments
                            return myTcb.getFd((fileSystem.open(s[0],s[1]))); //Return the TCB's user tid/pid
                        }else{
                    ...
                    case CLOSE:   // to be implemented in project
                        if ((myTcb = scheduler.getMyTcb()) != null) { //if the TCB actually exists
                            ftEnt = myTcb.getFtEnt(param); //Get the file table entry
                            if (ftEnt == null || fileSystem.close(ftEnt) == false) { //If the file table entry doesn't exist or cannot close
                                return ERROR;   //Return an ERROR
                            }
                            if (myTcb.returnFd(param) != ftEnt) { //If the file descriptor wasn't removed to match theftEntry
                                return ERROR; //Return an ERROR
                            } else {
                                return OK; //Otherwise it closed successfully to return OK
                            }
                        }
                        return ERROR; //Return an ERROR if the TCB doesn't exist
                    case SIZE:    // to be implemented in project
                        if ((myTcb = scheduler.getMyTcb()) != null) { //If the TCB actually exists
                            ftEnt = myTcb.getFtEnt(param); //Put the file table entry in the file table
                            if (ftEnt != null) { //If it was successfully placed, return the size
                                return fileSystem.fsize(ftEnt);
                            }
                        }
                        return ERROR; //Otherwise return error
                    case SEEK:    // to be implemented in project
                        if ((myTcb = scheduler.getMyTcb()) != null) { //If the TCB actually exists
                            int[] seekArgs = (int[])args; //Create an integer array from args
                            FileTableEntry ftEnt2 = myTcb.getFtEnt(param);  //Get the filetableentry at the given index
                            if (ftEnt2 != null) { //As long as the file table entry exists
                                return fileSystem.seek(ftEnt2, seekArgs[0], seekArgs[1]); //Use file system to randomly
                                // access a pointer in the file similar to a seekable channel
                            }
                        }
                        return ERROR; //Otherwise return an ERROR
                    case FORMAT:  // to be implemented in project
                        return (fileSystem.format(param) == true) ? OK : ERROR; //Return OK if the format was successful
                    case DELETE:  // to be implemented in project
                        return (fileSystem.delete((String)args) == true) ? OK : ERROR; //Return OK if the delete was successful
                }
                return ERROR;
